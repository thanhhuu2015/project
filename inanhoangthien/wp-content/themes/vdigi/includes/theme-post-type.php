...
if (!class_exists('VdigiCustomPostType')) {
    class VdigiCustomPostType
    {
        ...
        {
            $this->create_cpt_services();
            $this->create_cpt_products();
        }

        /**
         * Khu vực tạo các hàm trạng thái
         */
        public function create_cpt_services()
        {
            add_action('init', array($this, 'custom_post_type_of_v_services'), 0);
//            add_action('init', array($this, 'custom_taxonomy_of_v_services'), 0);
            //Các phần cấu hình cho post type và categry vừa tạo nếu có taxonomy
//            new VdigiCustomPostTypeConfig('v_services', 'v_category_of_services');
        }

        public function create_cpt_products()
        {
            add_action('init', array($this, 'custom_post_type_of_v_products'), 0);
        }

        /**
         * Khu vực tạo các hàm thực thi
         */
        public function custom_post_type_of_v_services()
        {
            $labels = array(
                'name' => _x('Dịch vụ', 'Post Type General Name', 'vdigi'),
                'singular_name' => _x('Dịch vụ', 'Post Type Singular Name', 'vdigi'),
                'menu_name' => __('Dịch vụ', 'vdigi'),
                'name_admin_bar' => __('Dịch vụ', 'vdigi'),
                'archives' => __('Item Archives', 'vdigi'),
                'attributes' => __('Item Attributes', 'vdigi'),
                'parent_item_colon' => __('Parent Item:', 'vdigi'),
                'all_items' => __('Tất cả dịch vụ', 'vdigi'),
                'add_new_item' => __('Thêm mới dịch vụ', 'vdigi'),
                'add_new' => __('Thêm mới dịch vụ', 'vdigi'),
                'new_item' => __('Dịch vụ mới', 'vdigi'),
                'edit_item' => __('Sửa dịch vụ', 'vdigi'),
                'update_item' => __('Cập nhật dịch vụ', 'vdigi'),
                'view_item' => __('Xem dịch vụ', 'vdigi'),
                'view_items' => __('Xem dịch vụ', 'vdigi'),
                ...
                'insert_into_item' => __('Chèn dịch vụ', 'vdigi'),
                'uploaded_to_this_item' => __('Upload vào dịch vụ', 'vdigi'),
                'items_list' => __('Danh sách dịch vụ', 'vdigi'),
                'items_list_navigation' => __('Danh sách dịch vụ', 'vdigi'),
                'filter_items_list' => __('Lọc danh sách dịch vụ', 'vdigi'),
            );

            $rewrite = array(
                'slug' => _x('dich-vu', 'slug', 'vdigi'), //Slug của trang chi tiết Dịch vụ
                'with_front' => true,
                'pages' => true,
                'feeds' => true,
            );
            $args = array(
                'label' => __('Dịch vụ', 'vdigi'),
                'description' => __('Tạo bài viết vào mục dịch vụ', 'vdigi'),
                'labels' => $labels,
                'supports' => array('title', 'editor', 'thumbnail', 'comments', 'trackbacks', 'revisions', 'custom-fields', 'page-attributes', 'post-formats'),
                'taxonomies' => ['v_category_of_services'],
            ...
            register_post_type('v_services', $args);
        }

        public function custom_taxonomy_of_v_services()
        {
            $labels = array(
                'name' => _x('Danh mục dịch vụ', 'Taxonomy General Name', 'vdigi'),
                'singular_name' => _x('Danh mục dịch vụ', 'Taxonomy Singular Name', 'vdigi'),
                'menu_name' => __('Danh mục dịch vụ', 'vdigi'),
            ...
            register_taxonomy('v_category_of_services', array('v_services'), $args);
        }

        //products
        public function custom_post_type_of_v_products()
        {
            $labels = array(
                'name' => _x('Sản phẩm', 'Post Type General Name', 'vdigi'),
                'singular_name' => _x('Sản phẩm', 'Post Type Singular Name', 'vdigi'),
                'menu_name' => __('Sản phẩm', 'vdigi'),
                'name_admin_bar' => __('Sản phẩm', 'vdigi'),
                'archives' => __('Item Archives', 'vdigi'),
                'attributes' => __('Item Attributes', 'vdigi'),
                'parent_item_colon' => __('Parent Item:', 'vdigi'),
                'all_items' => __('Tất cả Sản phẩm', 'vdigi'),
                'add_new_item' => __('Thêm mới Sản phẩm', 'vdigi'),
                'add_new' => __('Thêm mới Sản phẩm', 'vdigi'),
                'new_item' => __('Sản phẩm mới', 'vdigi'),
                'edit_item' => __('Sửa dịch vụ', 'vdigi'),
                'update_item' => __('Cập nhật Sản phẩm', 'vdigi'),
                'view_item' => __('Xem Sản phẩm', 'vdigi'),
                'view_items' => __('Xem Sản phẩm', 'vdigi'),
                'search_items' => __('Tìm kiếm', 'vdigi'),
                'not_found' => __('Không tìm thấy', 'vdigi'),
                'not_found_in_trash' => __('Không tìm thấy trong thùng rác', 'vdigi'),
                'featured_image' => __('Ảnh đại diện', 'vdigi'),
                ...
                'insert_into_item' => __('Chèn dịch vụ', 'vdigi'),
                'uploaded_to_this_item' => __('Upload vào dịch vụ', 'vdigi'),
                'items_list' => __('Danh sách Sản phẩm', 'vdigi'),
                'items_list_navigation' => __('Danh sách Sản phẩm', 'vdigi'),
                'filter_items_list' => __('Lọc danh sách Sản phẩm', 'vdigi'),
            );

            $rewrite = array(
                'slug' => _x('san-pham', 'slug', 'vdigi'), //Slug của trang chi tiết Dịch vụ
                'with_front' => true,
                'pages' => true,
                'feeds' => true,
            );
            $args = array(
                'label' => __('Sản phẩm', 'vdigi'),
                'description' => __('Tạo bài viết vào mục Sản phẩm', 'vdigi'),
                'labels' => $labels,
                'supports' => array('title', 'editor', 'thumbnail', 'comments', 'trackbacks', 'revisions', 'custom-fields', 'page-attributes', 'post-formats'),
//              'taxonomies' => '',
                'hierarchical' => true,  //Cho phép phân cấp, nếu là false thì post type này giống như Post, true thì giống như Page
                ...
                'menu_icon' => 'dashicons-networking', //Đường dẫn tới icon sẽ hiển thị, tại https://developer.wordpress.org/resource/dashicons/
                'show_in_admin_bar' => true,   //Hiển thị trong Appearance -> Menus
                'show_in_nav_menus' => true,
                'can_export' => true,   //Có thể export nội dung bằng Tools -> Export
                'has_archive' => true,   //Cho phép lưu trữ (month, date, year)
            ...
            register_post_type('v_products', $args);
        }
    }
...
//{
//     unregister_post_type('v_category_of_services');
//}
//
//$delete = add_action('init','v_delete_post_type'); var_dump($delete); die('ok');