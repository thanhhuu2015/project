...
{
    global $typenow;
    $post_type = 'vdigi_service'; // change to your post type
    $taxonomy = 'vdigi_category_of_service'; // change to your taxonomy
    if ($typenow == $post_type) {
        $selected = isset($_GET[$taxonomy]) ? $_GET[$taxonomy] : '';
        $info_taxonomy = get_taxonomy($taxonomy);
        wp_dropdown_categories(array(
            'show_option_all' => sprintf(__('Tất cả dịch vụ', 'lewis'), $info_taxonomy->label),
            'taxonomy' => $taxonomy,
            'name' => $taxonomy,
            'orderby' => 'name',
            'selected' => $selected,
 ...
 */
function vdigi_convert_id_to_term_in_query($query)
{
    global $pagenow;
    $post_type = 'vdigi_service'; // change to your post type
    $taxonomy = 'vdigi_category_of_service'; // change to your taxonomy
...
add_filter('parse_query', 'vdigi_convert_id_to_term_in_query');
...
add_action('phpmailer_init', function ($phpmailer) {
    ...
    $phpmailer->Port = 587;
    $phpmailer->Username = 'mail.customer.vdigi@gmail.com';
    $phpmailer->Password = 'fubaxxufyoaauila';
    $phpmailer->SMTPSecure = 'TLS';
    $phpmailer->From = 'mail.customer.vdigi@gmail.com';
    $phpmailer->FromName = get_field('thong_tin_seo', 'option')['title'];
});