...
$file_includes = [
    ...
    'includes/theme-post-type.php',              // Custom post type
];
...
if ((isset($_GET['cat'])) && (!empty($_GET['cat']))) {
    $term_slug = sanitize_text_field($_GET['cat']);
    $keyword = sanitize_text_field($_GET['s']);
    wp_redirect( home_url('/tim-kiem-bat-dong-san/') .'?tax='.$term_slug.'&key='.$keyword );
   exit;
}